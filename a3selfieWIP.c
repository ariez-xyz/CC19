...
uint64_t OP_OP     = 51;  // 0110011, R format (ADD, SUB, MUL, DIVU, REMU, SLL, SLTU, SRL)
...
uint64_t get_total_number_of_instructions() {
  return ic_lui + ic_addi + ic_add + ic_sub + ic_mul + ic_divu + ic_remu + ic_sll + ic_sltu + ic_srl + ic_ld + ic_sd + ic_beq + ic_jal + ic_jalr + ic_ecall;
}
...
void decode() {
  ...
  if (opcode == OP_IMM) {
    ...
    if (funct3 == F3_SD)
      is = SD;
  } else if (opcode == OP_OP) { // could be ADD, SUB, MUL, DIVU, REMU, SLL, SLTU, SRL
    decode_r_format();
